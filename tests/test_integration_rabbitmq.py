...
from protobunny.backends.rabbitmq.connection import AsyncConnection, SyncConnection
...
class TestIntegration:
    ...
    async def setup_connections(self, mocker: MockerFixture) -> tp.AsyncGenerator[None, None]:
        ...
        assert isinstance(await queue.get_connection(), AsyncConnection)
        await pb.unsubscribe_all()
        yield
        await connection.disconnect()
        AsyncConnection._instance_by_vhost.clear()
...
class TestIntegrationSync:
    ...
    @pytest.fixture(autouse=True)
    def setup_connections(self) -> tp.Generator[None, None, None]:
        ...
        conn = rabbitmq_backend.connection.get_connection_sync()
